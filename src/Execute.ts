...
class Execute {
  ...
  async menu(contaEncontrada: ContaCorrente | ContaPoupanca) {
    let option: string = "";
    do {
      ...
      switch (opcao.menu) {
          ...
          const withdrawal = await inquirer.prompt([
            {
              ...
              type: "number",
              message:
                "Digite um valor para realizar um saque na conta selecionada.",
            },
          ]);
        ...
        case "Transferir":
          ...
          if (
            typeof transfer.transfer_value == "number" &&
            typeof transfer.transfer_account == "number"
          ) {
            const contaATransferir = this.banco.procurarConta(
              transfer.transfer_account
            );
            if (contaATransferir instanceof ContaBancaria) {
              contaEncontrada?.transferir(
