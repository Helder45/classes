    ...
    if (account_type === "Conta Poupança") {
      ...
      try {
        if (!isNaN(poupanca.account_number) && !isNaN(poupanca.account_tax)) {
          this.banco.inserir(
            new ContaPoupanca(poupanca.account_number, 0, poupanca.account_tax)
          );
  ...
  async selecionarConta() {
    ...
    if (contaEncontrada instanceof ContaCorrente || ContaPoupanca) {
      ...
      do{
        ...
        switch (opcao.menu) {
          case "Depositar":
            const deposit = await inquirer.prompt([{name: "deposit_value", type: "number", message: "Digite um valor para depositar na conta selecionada."},]);
            ...
            break;
          case "Sacar":
            const withdrawal = await inquirer.prompt([{name: "withdrawal_value", type: "number", message: "Digite um valor para realizar um saque na conta selecionada."},]);
            ...
            break;
          case "Transferir":
            const transfer = await inquirer.prompt([{name: "transfer_value", type: "number", message: "Digite um valor para transferir da conta selecionada."},
          {name: "transfer_account", type: "number", message: "Digite o número da conta a receber a transferência:"},]);

          if (typeof transfer.transfer_value == 'number' && typeof transfer.transfer_account == 'number') {
          ...
          case "Gerar Relatório":
            const relatorioIndividual = new Relatorio();
            // relatorioIndividual.gerarRelatorio(contaEncontrada);
            break;
